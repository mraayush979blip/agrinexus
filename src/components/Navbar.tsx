...
const Navbar = () => {
  ...
  // Always add Profile
  navItems.push({ icon: () => <div className="w-6 h-6 rounded-full overflow-hidden border border-slate-200 shadow-sm"><img src={profile?.photoURL || `https://api.dicebear.com/7.x/avataaars/svg?seed=${profile?.name}`} alt="P" className="w-full h-full object-cover" /></div>, label: 'Profile', href: '/profile' });
        ...
        return (
          ...
          >
            <div className={cn(
              ...
              {typeof Icon === 'function' ? <Icon /> : <Icon size={24} strokeWidth={isActive ? 2.5 : 2} />}
            </div>
